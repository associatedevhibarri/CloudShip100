...
const courierStatusToStage = (courierStatus) => COURIER_STAGE[normalizeCourierStatus(courierStatus)] || null;

const bookingStatusFromStage = (stage) => {
  if (stage === 'delivered') return 'completed';
  if (stage === 'in_transit' || stage === 'out_for_delivery' || stage === 'warehouse') return 'in_transit';
  return 'pending';
};

const displayShipmentStatus = (booking) => {
  const fromCourier = courierStatusToStage(booking && booking.courierStatus);
  if (fromCourier) return fromCourier;
  if (booking && (booking.status === 'completed' || booking.status === 'history')) return 'delivered';
  if (booking && booking.status === 'delivered') return 'delivered';
  if (booking && (booking.logisticsBookingRef || booking.trackingNumber)) return 'booked';
  return (booking && booking.status) || 'pending';
};

const displayShipmentLabel = (booking) => {
  const ref = String((booking && (booking.logisticsBookingRef || booking.trackingNumber)) || '');
  if (ref.startsWith('STUB-') || ref.startsWith('TRK-STUB-')) {
    return 'courier book failed';
  }
  if (booking && booking.courierStatus) {
    return String(booking.courierStatus).replaceAll('_', ' ').replaceAll('-', ' ');
  }
  return displayShipmentStatus(booking);
};

const progressTimeline = (timeline, status, now) => {
  const current =
  ...
  return base.map((step) => {
    ...
    const label =
      step.label || (DEFAULT_TIMELINE.find((row) => row.stage === step.stage) || {}).label || step.stage;
    return {
      stage: step.stage,
      label,
      timestamp: done ? step.timestamp || now || null : step.timestamp || null,
      done,
    };
  });
};
